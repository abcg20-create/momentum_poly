...
const RUN_AUDIT_BACKGROUND_MAX_STOPPED_RUNS = Math.max(1, Number(process.env.RUN_AUDIT_BACKGROUND_MAX_STOPPED_RUNS || 2));
...
async function listActiveRunAuditBotsForWarm() {
  ...
  const sourceHostPort = normalizeRunAuditHostPort(LIVE_ONLY_SOURCE_HOST_PORT || RUN_AUDIT_HOST_PORT);
  if (sourceHostPort) reqUrl.searchParams.set("sourceHostPort", sourceHostPort);
...
async function listRecentlyStoppedRunAuditsForWarm() {
  if (!UPSTREAM_ORIGIN) return [];
  const sourceHostPort = normalizeRunAuditHostPort(LIVE_ONLY_SOURCE_HOST_PORT || RUN_AUDIT_HOST_PORT);
  const reqUrl = new URL("/api/compare/run-dashboard", "http://worker.local");
  const upstreamUrl = buildUpstreamUrl(reqUrl);
  const payload = await fetchJsonDirect(upstreamUrl.toString());
  ...
  return rows
    .map((row) => {
      ...
      const hostPort = normalizeRunAuditHostPort(row?.hostPort || sourceHostPort);
      const runNum = Number(row?.runNum || 0);
      ...
      if (!hostPort || (sourceHostPort && hostPort !== sourceHostPort)) return null;
      if (["running", "starting", "resuming"].includes(status)) return null;
      if (!(Number.isFinite(endedAtMs) && endedAtMs > 0)) return null;
      return {
        hostPort,
        runNum: Math.floor(runNum),
        runId,
        instanceId: instanceId || `stopped_run_${Math.floor(runNum)}`,
    ...
    .slice(0, RUN_AUDIT_BACKGROUND_MAX_STOPPED_RUNS);
}
...
function buildRunAuditWarmUrl(botLike) {
  ...
  reqUrl.searchParams.set("hostPort", normalizeRunAuditHostPort(bot.hostPort || RUN_AUDIT_HOST_PORT));
...
function buildSessionAuditWarmUrl(botLike, slugLike) {
  ...
  reqUrl.searchParams.set("sourceHostPort", normalizeRunAuditHostPort(bot.hostPort || RUN_AUDIT_HOST_PORT));
...
async function ensureSessionAuditArtifact(reqUrl) {
  ...
  if (preferLocalCompactFirst) {
    try {
      ...
      if (compact && typeof compact === "object" && !sessionAuditCompactLooksStale(compact) && compactPath) {
        ...
        persistRecoveredSessionAuditArtifacts(compact, { runNum, slug, hostPort: sourceHostPort });
      }
    } catch {}
  }
  if (!localCompactBuilt) {
    try {
      ...
      canonicalFetchError = String(error?.message || error || "session_audit_warm_failed");
    }
  }
  try {
  ...
  return {
    ok: localCompactBuilt || (canonicalStatusCode >= 200 && canonicalStatusCode < 300),
    ...
    compactPath,
    localCompactBuilt,
    canonicalFetchError: canonicalFetchError || null,
  };
}
...
function sessionAuditCompactLooksStale(compactLike) {
  ...
  const compactSideTimelineCount = ["UP", "DOWN"].reduce((count, side) => (
    count + (Array.isArray(compact?.sideAudit?.[side]?.timeline) ? compact.sideAudit[side].timeline.length : 0)
  ), 0);
  const compactMergedTimelineCount = Array.isArray(compact?.timeline) ? compact.timeline.length : 0;
  const compactTimelineCount = Math.max(compactSideTimelineCount, compactMergedTimelineCount);
  const compactTraceCount = Array.isArray(compact?.trace?.xMs) ? compact.trace.xMs.length : 0;
  const compactMarkedNoTrade = compact?.sessionSummary?.noTrade === true;
  if (compactSource === "degraded_endpoint_fallback") return true;
  if (compactTradeCount > 0 && compactTimelineCount === 0) return true;
  if (
    compactSource === "session_history_direct_fallback" &&
    compactTradeCount === 0 &&
    compactTimelineCount === 0 &&
    !compactMarkedNoTrade
  ) {
    return true;
  }
  if (
...
function localHostSessionAuditReviewPath(runNumLike, slugLike, hostPortLike = "") {
  ...
  const safeHost = normalizeRunAuditHostPort(hostPortLike || RUN_AUDIT_HOST_PORT);
  if (!(Number.isFinite(runNum) && runNum > 0) || !slug) return "";
  return path.join(runAuditSourceMirrorDir(runNum, safeHost), "session_audits", `${slug}.review.html`);
}

function localHostSessionAuditCompactPath(runNumLike, slugLike, hostPortLike = "") {
  const runNum = Math.floor(Number(runNumLike));
  const slug = String(slugLike || "").trim();
  const safeHost = normalizeRunAuditHostPort(hostPortLike || RUN_AUDIT_HOST_PORT);
  if (!(Number.isFinite(runNum) && runNum > 0) || !slug) return "";
...
function localHostSessionAuditRealFillsPath(runNumLike, slugLike, hostPortLike = "") {
  const runNum = Math.floor(Number(runNumLike));
  const slug = String(slugLike || "").trim();
  const safeHost = normalizeRunAuditHostPort(hostPortLike || RUN_AUDIT_HOST_PORT);
  if (!(Number.isFinite(runNum) && runNum > 0) || !slug) return "";
  return path.join(runAuditSourceMirrorDir(runNum, safeHost), "session_audits", `${slug}.real_fills.json`);
}

function canonicalTradeLogHostRoot(hostPortLike = "") {
  if (!RUN_AUDIT_LOCAL_FS_ENABLED || !RUN_AUDIT_REMOTE_ROOT) return "";
  const safeHost = normalizeRunAuditHostPort(hostPortLike || RUN_AUDIT_HOST_PORT);
  if (!safeHost) return "";
  return path.join(RUN_AUDIT_REMOTE_ROOT, "trade_logs", "hosts", `host_${safeHost}`);
}

function normalizeSessionAuditRealFillRole(actionLike = "") {
  const action = String(actionLike || "").trim().toUpperCase();
  if (!action) return "unknown";
  if (action.includes("ENTER")) return "entry";
  if (action.includes("EXIT") || action.includes("STOP") || action.includes("TP")) return "exit";
  return "unknown";
}

function normalizeSessionAuditRealFillLifecycle(actionLike = "") {
  const action = String(actionLike || "").trim().toUpperCase();
  if (!action) return "unknown";
  if (action.includes("SIGNAL")) return "signal";
  if (action.includes("FAILED") || action.includes("REJECT") || action.includes("BLOCKED")) return "failed";
  if (action.includes("SUBMIT_START")) return "submit";
  if (action.includes("ORDER")) return "submit";
  if (action.includes("FIRST_FILL_SEEN")) return "fill_seen";
  if (action.includes("FILL_CONFIRMED")) return "fill_confirmed";
  if (
    /^ENTER_[A-Z_]+_LIVE$/.test(action) ||
    /^EXIT_[A-Z_]+_LIVE$/.test(action) ||
    /^STOP_[A-Z_]+_LIVE$/.test(action)
  ) {
...
function buildCanonicalSessionRealFillArtifact(slugLike, runNumLike, hostPortLike = "") {
  const slug = String(slugLike || "").trim();
  const runNum = Math.floor(Number(runNumLike));
  const safeHost = normalizeRunAuditHostPort(hostPortLike || RUN_AUDIT_HOST_PORT);
  const hostRoot = canonicalTradeLogHostRoot(safeHost);
  if (!(Number.isFinite(runNum) && runNum > 0) || !slug || !hostRoot || !fs.existsSync(hostRoot)) return null;
  let fileNames = [];
  try {
    fileNames = fs.readdirSync(hostRoot)
      .filter((name) => /_trades\.jsonl$/i.test(String(name || "")))
      .sort();
  } catch {
    return null;
  }
  ...
  for (const fileName of fileNames) {
    const filePath = path.join(hostRoot, fileName);
    let fileText = "";
    try {
    ...
    for (const lineRaw of String(fileText || "").split(/\r?\n/)) {
      ...
      const marketSlug = String(row?.market?.slug || row?.marketSlug || row?.slug || "").trim();
      if (marketSlug !== slug) continue;
      const engine = String(row?.engine || "").trim().toLowerCase();
      const executionMode = String(row?.executionMode || row?.mode || "").trim().toLowerCase();
      if (engine !== "live" && executionMode !== "real") continue;
      const action = String(row?.action || "").trim();
      if (!action) continue;
      const st = row?.st && typeof row.st === "object" ? row.st : {};
      const execution = row?.execution && typeof row.execution === "object" ? row.execution : {};
      const signalTsMs = Number(
        row?.signalTsMs ??
        execution?.signalTsMs ??
        st?.signalTsMs ??
        0
      );
      const submittedTsMs = Number(
        row?.orderPlacedAtMs ??
        execution?.orderPlacedAtMs ??
        st?.orderPlacedAtMs ??
        0
      );
      const fillTsMs = Number(
        execution?.actualFillTsMs ??
        row?.actualFillTsMs ??
        st?.entryTsMs ??
        st?.exitTsMs ??
        row?.t ??
      ...
      const canonicalTsMs = [signalTsMs, submittedTsMs, fillTsMs]
        .find((value) => Number.isFinite(Number(value)) && Number(value) > 946684800000);
      if (!Number.isFinite(Number(canonicalTsMs))) continue;
      const fillPrice = Number(
        row?.fillPx ??
        row?.actualFillPx ??
        execution?.actualFillPx ??
        execution?.actualPx ??
        st?.entryPx ??
        st?.exitPx ??
        0
      );
      const fillSize = Number(
        row?.filledShares ??
        row?.soldShares ??
        row?.sharesClosed ??
        st?.sharesClosed ??
      ...
      const role = normalizeSessionAuditRealFillRole(action);
      const feeUsd = Number(
        role === "entry"
          ? (st?.entryFeeUsd ?? row?.entryFeeUsd ?? execution?.entryFeeUsd)
          : (st?.exitFeeUsd ?? row?.exitFeeUsd ?? execution?.exitFeeUsd)
      );
      rows.push({
        slug,
        runNum,
        hostPort: safeHost,
        sourceFile: fileName,
        iso: String(row?.iso || "").trim() || null,
        action,
        role,
        lifecycle: normalizeSessionAuditRealFillLifecycle(action),
        side: String(execution?.side || st?.side || row?.side || "").trim().toUpperCase() || null,
        signalTsMs: Number.isFinite(signalTsMs) && signalTsMs > 946684800000 ? signalTsMs : null,
        submittedTsMs: Number.isFinite(submittedTsMs) && submittedTsMs > 946684800000 ? submittedTsMs : null,
        fillTsMs: Number.isFinite(fillTsMs) && fillTsMs > 946684800000 ? fillTsMs : null,
        canonicalTsMs: Number(canonicalTsMs),
        signalPx: Number.isFinite(Number(execution?.signalPx)) ? Number(execution.signalPx) : null,
        intendedPx: Number.isFinite(Number(execution?.intendedPx)) ? Number(execution.intendedPx) : null,
        fillPrice: Number.isFinite(fillPrice) ? fillPrice : null,
        fillSize: Number.isFinite(fillSize) ? fillSize : null,
        feeUsd: Number.isFinite(feeUsd) ? feeUsd : null,
        orderId: String(row?.orderId || row?.stopOrderId || row?.tpOrderId || "").trim() || null,
        tradeKey: String(row?.tradeKey || "").trim() || null,
        rejectReason: String(row?.rejectReason || row?.error || row?.reason || "").trim() || null,
        stopReason: String(row?.stopReason || row?.execution?.stopReason || "").trim() || null,
      });
    }
  }
  if (!rows.length) return null;
  rows.sort((a, b) => Number(a?.canonicalTsMs || 0) - Number(b?.canonicalTsMs || 0));
  return {
    schemaVersion: "session-audit-real-fills.v1",
    slug,
    runNum,
    hostPort: safeHost,
    source: "readonly_worker_host_trade_jsonl_live",
    generatedAtMs: Date.now(),
    rows,
  };
}

function persistCanonicalSessionRealFillArtifact(slugLike, runNumLike, hostPortLike = "") {
  const artifact = buildCanonicalSessionRealFillArtifact(slugLike, runNumLike, hostPortLike);
  if (!artifact) return false;
  const outPath = localHostSessionAuditRealFillsPath(runNumLike, slugLike, hostPortLike);
...
function persistRecoveredSessionAuditArtifacts(compactLike, meta = {}) {
  const compact = compactLike && typeof compactLike === "object" ? compactLike : null;
  if (!compact || sessionAuditCompactLooksStale(compact)) return false;
  const runNum = Math.floor(Number(meta?.runNum));
  const slug = String(meta?.slug || compact?.slug || "").trim();
  const hostPort = normalizeRunAuditHostPort(meta?.hostPort || RUN_AUDIT_HOST_PORT);
  if (!(Number.isFinite(runNum) && runNum > 0) || !slug) return false;
  const compactPath = localHostSessionAuditCompactPath(runNum, slug, hostPort);
  const reviewPath = localHostSessionAuditReviewPath(runNum, slug, hostPort);
  if (!compactPath || !reviewPath) return false;
  try {
    const reviewHtml = renderSessionAuditCompactHtml(compact, { runNum, slug, hostPort });
    fs.mkdirSync(path.dirname(compactPath), { recursive: true });
    fs.writeFileSync(compactPath, `${JSON.stringify(compact, null, 2)}\n`, "utf8");
...
function buildCompactPayloadFromSessionRow(rowLike, sourceTag = "session_history_fallback") {
  ...
  const sideAuditTimelineCount = ["UP", "DOWN"].reduce(
    (count, side) => count + (Array.isArray(sideAudit?.[side]?.timeline) ? sideAudit[side].timeline.length : 0),
    0
  );
  const fallbackTimeline = sideAuditTimelineCount > 0
    ? []
    : recoveredTradeSummary.flatMap((trade, idx) => {
        const tradeNum = Number.isFinite(Number(trade?.tradeNum)) ? Number(trade.tradeNum) : idx + 1;
        const sideText = String(trade?.side || row?.side || row?.tradeSide || "").trim().toUpperCase() || null;
        const tradeParadigm = String(trade?.tradeParadigm || `Recovered Trade ${tradeNum}`).trim() || `Recovered Trade ${tradeNum}`;
        const entryTsMs = Number(trade?.entryTsMs ?? startMs);
        const exitTsMs = Number(trade?.exitTsMs ?? endMs);
        const rows = [];
        if (Number.isFinite(entryTsMs) && entryTsMs > 946684800000) {
          rows.push({
            ...
            fillTsMs: entryTsMs,
            event: "enter",
            entryPx: Number.isFinite(Number(trade?.entryPx)) ? Number(trade.entryPx) : null,
            actualFillPx: Number.isFinite(Number(trade?.entryPx)) ? Number(trade.entryPx) : null,
            fillPx: Number.isFinite(Number(trade?.entryPx)) ? Number(trade.entryPx) : null,
            signalPx: Number.isFinite(Number(trade?.entryPx)) ? Number(trade.entryPx) : null,
            intendedPx: Number.isFinite(Number(trade?.entryPx)) ? Number(trade.entryPx) : null,
            executionMode: "paper",
            fillSource: "session_summary_recovery",
            reason: String(trade?.reason || "Recovered entry from session summary.").trim(),
            feesUsd: null,
            pnlUsd: null,
          });
        }
        if (Number.isFinite(exitTsMs) && exitTsMs > 946684800000) {
          rows.push({
            side: sideText,
            tradeNum,
            tradeParadigm,
            tsMs: exitTsMs,
            fillTsMs: exitTsMs,
            event: String(trade?.exitType || "").toLowerCase().includes("partial") ? "exit_partial" : "exit",
            entryPx: Number.isFinite(Number(trade?.entryPx)) ? Number(trade.entryPx) : null,
            exitPx: Number.isFinite(Number(trade?.exitPx)) ? Number(trade.exitPx) : null,
            actualFillPx: Number.isFinite(Number(trade?.exitPx)) ? Number(trade.exitPx) : null,
            fillPx: Number.isFinite(Number(trade?.exitPx)) ? Number(trade.exitPx) : null,
            signalPx: Number.isFinite(Number(trade?.exitPx)) ? Number(trade.exitPx) : null,
            intendedPx: Number.isFinite(Number(trade?.exitPx)) ? Number(trade.exitPx) : null,
            executionMode: "paper",
            fillSource: "session_summary_recovery",
            reason: String(trade?.reason || "Recovered exit from session summary.").trim(),
            exitType: String(trade?.exitType || "session_close").trim() || "session_close",
            grossPnlUsd: Number.isFinite(Number(trade?.grossPnlUsd)) ? Number(trade.grossPnlUsd) : null,
            feesUsd: Number.isFinite(Number(trade?.feesUsd)) ? Number(trade.feesUsd) : null,
  ...
  return {
    ...
    sessionSummary: {
      ...
      laneCount: Number.isFinite(Number(row?.laneCount))
        ...
        : (Array.isArray(row?.lanes) ? row.lanes.length : null),
      source: sourceTag,
    },
...
function renderSessionAuditCompactHtml(compact, meta = {}) {
  ...
  const timelineRows = (
    Array.isArray(compact?.timeline) && compact.timeline.length > 0
      ? compact.timeline
      : ["UP", "DOWN"].flatMap((side) => {
    ...
    ? `<div class="card">
      ...
      ${tradeSummaries.map((trade, idx) => {
        ...
        return `<div class="tradeCard">
          ...
          <div class="meta">Entry: ${escapeHtmlLite(fmtTs(trade?.entryTsMs))} · ${escapeHtmlLite(fmtClock(trade?.entryTsMs))}</div>
...
async function syncRunAuditSourceMirror(runNum, remoteInputs, hostPort, opts = {}) {
  ...
  for (const session of detailedSessions) {
    ...
    try {
      // Build a canonical live-fill timeline from raw host trade logs on the
      // readonly worker. This keeps signal/submit/fill timestamps and sizes
      // off the hot path while giving Session Audit a stronger source than
      // continuity reconstruction for real executed trades.
      persistCanonicalSessionRealFillArtifact(slug, runNum, safeHost);
    } catch {}
  }
...
async function warmActiveRunAuditsInBackground() {
  ...
  try {
    ...
    const stoppedRuns = await listRecentlyStoppedRunAuditsForWarm();
    ...
    for (const bot of stoppedRuns) {
      const key = `stopped:${String(bot.hostPort || "")}:${String(bot.runId)}:${String(bot.runNum)}`;
      activeKeys.add(key);
      const previous = runAuditBackgroundWarmState.get(key) || null;
      const staleCheckDue = !previous || (Date.now() - Number(previous.lastAttemptAtMs || 0)) >= RUN_AUDIT_BACKGROUND_WARM_INTERVAL_MS;
      if (!staleCheckDue) continue;
      runAuditBackgroundWarmState.set(key, {
        ...previous,
        marketSlug: String(bot.marketSlug || ""),
        lastAttemptAtMs: Date.now(),
        ...
        runId: String(bot.runId || ""),
        instanceId: String(bot.instanceId || ""),
        hostPort: String(bot.hostPort || ""),
        pendingSessionAuditSlug: "",
      });
      pushActivity(
        `WARMING STOPPED RUN AUDIT FOR RUN ${String(bot.runNum)} ON HOST ${String(bot.hostPort || RUN_AUDIT_HOST_PORT)}`,
        {
          type: "run-audit-background-warm-stopped",
          runNum: String(bot.runNum),
          runId: String(bot.runId),
          instanceId: String(bot.instanceId),
          hostPort: String(bot.hostPort || RUN_AUDIT_HOST_PORT),
          marketSlug: String(bot.marketSlug || ""),
          trigger: "recent_stopped_run",
        },
      );
      const result = await ensureRunAuditArtifact(buildRunAuditWarmUrl(bot));
      runAuditBackgroundWarmState.set(key, {
