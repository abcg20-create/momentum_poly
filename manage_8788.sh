...
SCRIPT_PATH="${ROOT_DIR}/manage_8788.sh"
PORT="${PORT:-8788}"
LOCK_DIR="/tmp/polymarket_8788_supervisor.lockdir"
SUP_PID_FILE="/tmp/polymarket_8788_supervisor.pid"
CHILD_PID_FILE="/tmp/polymarket_8788_child.pid"
CHILD_START_TS_FILE="/tmp/polymarket_8788_child_start_ts"
HEARTBEAT_FILE="/tmp/polymarket_8788_supervisor.heartbeat"
LOG_FILE="/tmp/polymarket_8788_supervisor.log"
SERVER_LOG_FILE="/tmp/derisk_8788.log"
RESTART_REASON_FILE="/tmp/polymarket_8788_restart_reasons.jsonl"
PRE_RESTART_SNAPSHOT_FILE="/tmp/polymarket_8788_pre_restart.json"
RESTART_MARK_FILE="/tmp/polymarket_8788_restarting"
...
MAX_BAD_HEALTH="${MAX_BAD_HEALTH:-24}"
...
HEALTH_EXPECT_STREAM_ENABLED="${HEALTH_EXPECT_STREAM_ENABLED:-0}"
...
MAX_ENDPOINT_DOWN_FAILS="${MAX_ENDPOINT_DOWN_FAILS:-120}"
HEAL_REQUEST_COOLDOWN_SEC="${HEAL_REQUEST_COOLDOWN_SEC:-1}"
MAX_DEGRADED_HEALTH_FAILS="${MAX_DEGRADED_HEALTH_FAILS:-60}"
RESTART_SAFE_WAIT_SEC="${RESTART_SAFE_WAIT_SEC:-60}"
...
find_supervisor_pids() {
  pgrep -f "${SCRIPT_PATH} __supervise" 2>/dev/null || true
}
...
terminate_pid_tree() {
  local root_pid="${1:-}"
  local tag="${2:-pid_tree_terminate}"
  ...
  if [[ "${#descendants[@]}" -gt 0 ]]; then
    log_line "${tag}_descendants root=${root_pid} pids=$(printf '%s ' "${descendants[@]}")"
...
is_health_ok() {
  ...
  if [[ "${HEALTH_EXPECT_STREAM_ENABLED}" == "1" ]]; then
...
health_snapshot_json() {
  local json=""
  json="$(curl -fsS -m "${HTTP_TIMEOUT_SEC}" "${HEALTH_URL}" 2>/dev/null)" || return 1
  jq -c --argjson expectStream "${HEALTH_EXPECT_STREAM_ENABLED}" '{
    t: now | todate,
    ...
    streamEnabledExpected: ($expectStream == 1),
...
health_issue_codes() {
  ...
  jq -r --argjson maxStream "${MAX_STREAM_AGE_MS}" --argjson maxPair "${MAX_PAIR_LAG_MS}" --argjson maxGap "${MAX_QUOTE_GAP_MS}" --argjson maxStreak "${MAX_QUOTE_KPI_STREAK}" --argjson maxRuntime "${MAX_RUNTIME_LAG_MS}" --argjson quoteGapEnabled "$([[ "${QUOTE_BROADCAST_HEALTH_ENABLED}" == "1" ]] && echo true || echo false)" --argjson expectStream "${HEALTH_EXPECT_STREAM_ENABLED}" '
    [
      (if (.slug // "") == "" then "missing_slug" else empty end),
      (if ($expectStream == 1) and (.streamConnected // false) != true then "stream_disconnected" else empty end),
      (if ($expectStream == 1) and (.streamAgeMs != null) and ((.streamAgeMs | tonumber) > $maxStream) then "stream_age_high" else empty end),
...
ensure_nonessential_worker_supervisor() {
  local worker_origin="${MAIN_READ_ONLY_ORIGIN:-${READ_ONLY_ORIGIN:-http://127.0.0.1:9001}}"
  ...
  if curl -fsS --max-time 2 "${worker_origin%/}/api/health?lite=1" >/dev/null 2>&1; then
    log_line "ensure_nonessential_worker status=already_up"
    return 0
  fi
  log_line "ensure_nonessential_worker status=missing_local_worker origin=${worker_origin}"
  return 1
}
...
start_child() {
  ...
  (
    ...
    node_old_space_mb="${NODE_OLD_SPACE_MB:-4096}"
    exec env \
      NODE_OPTIONS="--max-old-space-size=${node_old_space_mb} ${NODE_OPTIONS:-}" \
      HOST="${APP_BIND_HOST:-127.0.0.1}" \
      BIND_HOST="${APP_BIND_HOST:-127.0.0.1}" \
      TRADE_LOG_NAMESPACE="host_8788" \
      TRADE_LOG_DIR="${ROOT_DIR}/trade_logs/hosts/host_8788" \
      MULTI_STATE_DB_PATH="${ROOT_DIR}/trade_logs/hosts/host_8788/20_multi_market_state.sqlite" \
      RESUME_LATEST_RUN_ON_RESTART="1" \
      FORCE_NEW_SERVER_RUN_ON_START="0" \
      QUOTE_PUMP_INTERVAL_MS="50" \
      QUOTE_PUMP_HEALTHY_INTERVAL_MS="100" \
      QUOTE_BROADCAST_INTERVAL_MS="100" \
      QUOTE_CLIENT_BROADCAST_ENABLED="1" \
      QUOTE_STREAM_ENABLED="1" \
      USER_STREAM_ENABLED="0" \
      PRICES_CLIENT_BROADCAST_ENABLED="1" \
      PRICES_CLIENT_BROADCAST_INTERVAL_MS="200" \
      ...
      QUOTE_STREAM_FORCE_RECONNECT_COOLDOWN_MS="8000" \
      QUOTE_STREAM_CONNECT_GRACE_MS="8000" \
      QUOTE_STREAM_OPEN_WARMUP_MS="5000" \
      QUOTE_STREAM_STALE_RECONNECT_MS="8000" \
      QUOTE_STREAM_HARD_RECONNECT_MS="20000" \
      QUOTE_KPI_RECONNECT_MIN_FEED_LAG_MS="4000" \
      QUOTE_KPI_RECONNECT_MIN_STREAM_AGE_MS="6000" \
      QUOTE_KPI_RECONNECT_MIN_UNHEALTHY_MS="5000" \
      ...
      HTTP_TIMEOUT_SEC="10" \
      MAX_BAD_HEALTH="24" \
      MAX_ENDPOINT_DOWN_FAILS="120" \
      MAX_DEGRADED_HEALTH_FAILS="60" \
      RESTART_SAFE_WAIT_SEC="60" \
      IMMEDIATE_RESTART_ON_UNHEALTHY="0" \
      MAIN_LOOP_PERIOD_MS="50" \
      BOT_RUNTIME_POLL_MS="50" \
      BOT_REALTIME_QUOTE_TRIGGER_ENABLED="1" \
      BOT_REALTIME_TRIGGER_MIN_MS="50" \
      BOT_REALTIME_MAINTENANCE_POLL_MS="100" \
      ...
      QUOTE_MAX_STALE_MS="5000" \
      QUOTE_STREAM_HEALTHY_MAX_AGE_MS="250" \
      QUOTE_PAIR_SKEW_MAX_MS="2000" \
      QUOTE_FETCH_TIMEOUT_MS="1200" \
      QUOTE_FALLBACK_TRIGGER_MS="250" \
      QUOTE_FALLBACK_TIMEOUT_MS="1500" \
      QUOTE_FALLBACK_COOLDOWN_MS="250" \
      QUOTE_BROADCAST_HEALTH_ENABLED="0" \
      BOT_RUN_TELEMETRY_INTERVAL_MS="2000" \
      BOT_RUN_TRUTH_RECON_INTERVAL_MS="3000" \
      PRICES_HEALTH_BROADCAST_INTERVAL_MS="2500" \
      HEALTH_SNAPSHOT_CACHE_MS="250" \
      HOT_HOST_TRACE_LIGHT_MODE="0" \
      SESSION_TRACE_MAIN_LOOP_SAMPLE_MIN_MS="100" \
      ALGO_INPUT_TRACE_SAMPLE_MIN_MS="50" \
      SESSION_TRACE_WRITE_MIN_MS="50" \
      BTC_5M_RECORD_MIN_INTERVAL_MS="50" \
      HOT_SERVICE_MODE="1" \
      READ_ONLY_ORIGIN="${MAIN_READ_ONLY_ORIGIN:-${READ_ONLY_ORIGIN:-http://127.0.0.1:9001}}" \
      RUN_AUDIT_READ_ONLY_ORIGIN="${MAIN_RUN_AUDIT_READ_ONLY_ORIGIN:-${MAIN_READ_ONLY_ORIGIN:-${READ_ONLY_ORIGIN:-http://127.0.0.1:9001}}}" \
      RUN_AUDIT_WARM_ENABLED="0" \
      SESSION_CARD_MATERIALIZE_ENABLED="0" \
      PARA_SYNC_ENABLED="0" \
      SESSION_PARITY_ENABLED="0" \
      SESSION_PARITY_OHIO_BASE_URL="http://127.0.0.1:${PORT}" \
      SESSION_PARITY_DUBLIN_BASE_URL="http://127.0.0.1:29888" \
      SESSION_PARITY_DUBLIN_SSH_HOST="3.250.161.191" \
      SESSION_PARITY_DUBLIN_SSH_USER="ec2-user" \
      SESSION_PARITY_DUBLIN_SSH_KEY="/home/ec2-user/.ssh/dublin_parity_ed25519" \
      SESSION_PARITY_WARM_DEBOUNCE_MS="10000" \
      SESSION_PARITY_ACTIVE_ALLOW_START_MS="10000" \
      SESSION_PARITY_ACTIVE_ALLOW_END_MS="100000" \
      PAPER_MARKET_ENTRY_DELAY_MS="0" \
      PORT="${PORT}" \
      "${NODE_BIN}" --require tsx/cjs main_bot_server_strategy_isolated.ts >>"${SERVER_LOG_FILE}" 2>&1
  ) &
  local spawned="$!"
...
stop_child() {
  ...
  if [[ -f "${CHILD_PID_FILE}" ]]; then
    ...
    cpid="$(cat "${CHILD_PID_FILE}" 2>/dev/null || true)"
    if [[ -n "${cpid}" ]]; then
      terminate_pid_tree "${cpid}" "child_stop"
    fi
    rm -f "${CHILD_PID_FILE}"
  fi
  local lpids
...
start_supervisor() {
  if ! ensure_nonessential_worker_supervisor; then
    if [[ "${REQUIRE_NONESSENTIAL_WORKER_FOR_START}" == "1" ]]; then
      echo "Failed to ensure main readonly worker before starting ${PORT}. Check ${LOG_FILE}" >&2
      return 1
    fi
  ...
  if command -v setsid >/dev/null 2>&1; then
    env -u NICE nohup setsid "${SCRIPT_PATH}" __supervise >>"${LOG_FILE}" 2>&1 </dev/null &
  else
    env -u NICE nohup "${SCRIPT_PATH}" __supervise >>"${LOG_FILE}" 2>&1 </dev/null &
  fi
  local i=0
  while (( i < 40 )); do
    if [[ -f "${SUP_PID_FILE}" ]]; then
...
stop_supervisor() {
  ...
  if [[ -n "${spid}" ]]; then
    terminate_pid_tree "${spid}" "supervisor_stop"
  fi
  local other_sups
  other_sups="$(find_supervisor_pids | tr '\n' ' ' | xargs 2>/dev/null || true)"
  if [[ -n "${other_sups}" ]]; then
    local opid=""
    for opid in ${other_sups}; do
      [[ -n "${opid}" ]] || continue
      terminate_pid_tree "${opid}" "supervisor_stop_duplicate"
    done
  fi
  local i=0
  while (( i < 50 )); do
