...
resolve_dublin_upstream_origin() {
...
DUBLIN_UPSTREAM_ORIGIN="$(resolve_dublin_upstream_origin || true)"
if [[ -z "${DUBLIN_UPSTREAM_ORIGIN}" ]]; then
  echo "no live upstream available on 8791; production restart requires the live service and must not fall back to paper or tunnels" >&2
  exit 1
fi
