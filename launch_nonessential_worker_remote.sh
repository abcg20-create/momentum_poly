...
mkdir -p "$ROOT/logs" "$ROOT/cache_live"
...
if ! curl -fsS --max-time 5 "$(build_health_url "${LIVE_UPSTREAM_ORIGIN}" "${LIVE_UPSTREAM_PATH_PREFIX}")" >/dev/null \
  ...
  echo "no live upstream available at $(build_health_url "${LIVE_UPSTREAM_ORIGIN}" "${LIVE_UPSTREAM_PATH_PREFIX}") or $(build_state_url "${LIVE_UPSTREAM_ORIGIN}" "${LIVE_UPSTREAM_PATH_PREFIX}"); launch requires direct live connectivity" >&2
  exit 1
fi
